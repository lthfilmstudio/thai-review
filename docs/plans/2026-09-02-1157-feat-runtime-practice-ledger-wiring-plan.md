...
- **Deviation（2026-09-02 實作時）:** U5 只落地 ledger 側的邏輯（`ledger-mirror.js`、`ensureRuntimeLedgerContext`、`resetRuntimeLedgerAuthority`、`planLegacyV1Import`／`commitLegacyV1Import`），**`src/cloud-sync.js` 與 `src/app.js` 的接線移到 U6**。在 U6 把評分接上 ledger 之前沒有任何東西寫 ledger，現在就把 practice port、catalog 與 lineage evidence 注進 sync 路徑等於在 production 同步流程上加一條沒人讀的相依，純風險零收益。U6 開工時一併接。
